...
def parse_cutoffs_from_pdf(path: str) -> pd.DataFrame:
    """
    Intenta extraer una tabla simple de notas de corte desde el PDF del Distrito Único de Madrid.
    Estrategia:
      - leer texto página a página
      - detectar universidad por líneas que contengan "UNIVERSIDAD ..."
      - capturar filas del tipo: <código> <titulación> <nota> ...
    Devuelve columnas: Universidad, Código, Grado, Nota (primer valor numérico de la fila).
    """
    try:
        import pdfplumber  # type: ignore
    except Exception:
        return pd.DataFrame(columns=["Universidad", "Código", "Grado", "Nota"])
    ...
    # patrón: código + nombre + primera nota (con coma o punto)
    row_re = re.compile(r"^(?P<cod>\d{3,4})\s+(?P<grado>.+?)\s+(?P<nota>\d{1,2}[.,]\d{2,3})\b")
        ...
        for page in pdf.pages:
            ...
            for raw in text.splitlines():
                ...
                if line.startswith("UNIVERSIDAD "):
                    current_uni = line.strip()
                    continue

                # A veces aparece sin empezar al inicio, lo capturamos igual
                if "UNIVERSIDAD " in line and line.isupper() and len(line) < 80:
                    if line.strip().startswith("UNIVERSIDAD "):
                        current_uni = line.strip()

                m = row_re.match(line)
                if m:
                    cod = m.group("cod")
                    grado = m.group("grado").strip()
                    nota = norm_num(m.group("nota"))
                    if nota is None:
                        continue
                    rows.append(
                        {
                            "Universidad": current_uni or "—",
                            "Código": cod,
                            "Grado": grado,
                            "Nota": nota,
                        }
    ...
    # Deduplicar quedándonos con la nota máxima por (Universidad, Grado)
    dfc = dfc.sort_values("Nota", ascending=False).drop_duplicates(["Universidad", "Grado"], keep="first")
    return dfc.reset_index(drop=True)


def match_uni_label(uni_from_excel: str) -> str:
    """
    Mapear abreviaturas del Excel (UCM, UAM, URJC...) a nombres del PDF.
    Ajusta aquí si detectas alguna diferencia.
    """
    if not isinstance(uni_from_excel, str) or not uni_from_excel:
        return ""
    u = uni_from_excel.strip().upper()
    mapping = {
...
with st.expander("Ver todas las columnas del Excel para los grados/universidades filtrados"):
    ...
    st.dataframe(df_scope[all_cols], use_container_width=True, hide_index=True)
...
with st.expander("Ver nota de corte por grado y universidad (según PDF DUM)"):
    if Path(PDF_CUTOFF_PATH).exists():
        cut = parse_cutoffs_from_pdf(PDF_CUTOFF_PATH)
        if cut.empty:
