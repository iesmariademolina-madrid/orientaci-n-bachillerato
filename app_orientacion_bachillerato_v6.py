...
# Flujo: el alumno elige 4 asignaturas (entre las que ponderan) y "descubre" la ponderación por grado/universidad.
# v6: Se elimina el filtro de "ponderaciones 0,2 / todas" y se corrige la propuesta de "4 mejores"
#     usando un ranking robusto (prioriza cuántas veces pondera 0,2 en lo seleccionado).
#
# Ejecuta (Windows recomendado): python -m streamlit run app_orientacion_bachillerato_v6.py
...
def order_subjects_best4(df_scope: pd.DataFrame, subjects: list[str]) -> list[str]:
    ...
    Prioriza:
      1) cuántas veces pondera 0,20 (más es mejor)
      2) cuántas veces pondera >0 (más es mejor)
      3) ponderación máxima (por si hubiera algo raro >0,2)
      4) ponderación media (desempate)
      5) nombre (estable)
    """
    stats = []
    for s in subjects:
        ...
        if col.dropna().empty:
            continue
        cnt_02 = int((col.round(3) == 0.2).sum())  # tolerante a flotantes
        cnt_nz = int((col > 0).sum())
        mx = float(col.max(skipna=True)) if not pd.isna(col.max(skipna=True)) else 0.0
        mean = float(col[col > 0].mean()) if (col > 0).any() else 0.0
        stats.append((s, cnt_02, cnt_nz, mx, mean))

    # ordenar (desc) por cnt_02, cnt_nz, mx, mean, luego nombre
    stats.sort(key=lambda t: (-t[1], -t[2], -t[3], -t[4], t[0]))
...
# Para que la lista de asignaturas sea realmente "solo las que ponderan", pedimos grados primero
...
st.caption("Solo aparecen asignaturas que ponderan (>0) en lo seleccionado. Elige exactamente 4 y descubrirás sus ponderaciones.")
...
with st.container(border=True):
    ...
    with b3:
        st.caption("Criterio: más veces con 0,20 en lo seleccionado; luego más veces >0; luego desempates.")
