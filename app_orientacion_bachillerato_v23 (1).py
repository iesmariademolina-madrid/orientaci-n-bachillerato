...
<style>
.table-wrap {{
  --c1: 340px;  /* ancho col Grado */
  --c2: 160px;  /* ancho col Universidad */
{{
...
.table-wrap table {{
  ...
  border-spacing: 0;
  width: max-content; /* fuerza scroll horizontal si hace falta */
  min-width: 100%;
  font-size: 0.84rem;
}}}}
...
/* colores de ponderación (relleno completo de celda) */
.table-wrap td.w20 {{ background: rgba(16,185,129,0.35); color: #064e3b; font-weight: 900; }}
.table-wrap td.w10 {{ background: rgba(245,158,11,0.35); color: #7c2d12; font-weight: 900; }}
.table-wrap td.w00 {{ background: rgba(148,163,184,0.18); color: rgba(15,23,42,0.65); font-weight: 800; }}

.table-wrap th:nth-child(1), .table-wrap th:nth-child(2) {{
  z-index: 10;
}}
</style>
""",
    ...
    # Colorear celdas (0,20 / 0,10 / 0)
    html2 = html
    # Ojo: to_html suele renderizar con punto decimal (0.2/0.1), pero aceptamos coma también.
    html2 = re.sub(r"<td>(0\.2(?:0)?|0,2(?:0)?)</td>", r"<td class='w20'>\1</td>", html2)
    html2 = re.sub(r"<td>(0\.1(?:0)?|0,1(?:0)?)</td>", r"<td class='w10'>\1</td>", html2)
    html2 = re.sub(r"<td>(0\.00|0,00|0)</td>", r"<td class='w00'>0</td>", html2)
...
# Agrupar por (Grado, Universidad) para evitar filas duplicadas del Excel
grp = df_scope.copy()
# Normalizamos nombres de columnas de salida
base = grp[["grado", "universidad"]].copy()
base = base.rename(columns={"grado": "Grado", "universidad": "Universidad"})

rows = []
for (g, u), block in grp.groupby(["grado", "universidad"], dropna=False):
    r = {"Grado": g, "Universidad": (u if (u is not None and str(u).strip() != "") else "—")}
    for s in chosen:
        col = pd.to_numeric(block[s], errors="coerce").fillna(0).round(2)
        # nos quedamos con el máximo (si hay varias filas del mismo grado+uni)
        v = float(col.max()) if not col.empty else 0.0
        r[s] = v if v in (0.1, 0.2) else 0.0
    rows.append(r)
...
with st.expander("Ver notas de corte del PDF (DUM NOTAS DE CORTE.pdf) para los grados/universidades filtrados"):
    if "load_cutoffs" not in globals():
        st.info("No está disponible el lector de notas de corte en esta versión.")
    else:
        df_cut = load_cutoffs(PDF_CUTOFF_PATH)
        if df_cut is None or df_cut.empty:
            st.info("No he podido extraer notas de corte del PDF. Asegúrate de que el archivo está en la misma carpeta y que el PDF contiene texto seleccionable (no escaneado).")
        else:
            # Filtrar por universidades seleccionadas (si hay) usando UNI_MAP si existe
            df_show = df_cut.copy()
            ...
            df_show["_u"] = df_show["Universidad"].map(_safe_norm)
            df_show["_g"] = df_show["Grado"].map(_safe_norm)

            sel_unis_pdf_frag = []
            for u in (selected_unis or []):
                frag = UNI_MAP.get(u, u) if "UNI_MAP" in globals() else u
                sel_unis_pdf_frag.append(_safe_norm(frag))

            if sel_unis_pdf_frag:
                df_show = df_show[df_show["_u"].apply(lambda x: any(f in x for f in sel_unis_pdf_frag))]

            sel_grados_norm = {_safe_norm(g) for g in selected_grados}
            if sel_grados_norm:
            ...
            st.download_button(
                "⬇️ Descargar notas de corte (CSV)",
                data=csv_cut,
                file_name="notas_corte_filtradas.csv",
                mime="text/csv",
            )
